...
type CartContextType = {
  items: CartLine[];
  loading: boolean;
  addToCart: (productId: string, variantId: string | null, quantity: number, stock: number) => Promise<void>;
...
// once, then cleared from localStorage — mirrors CartContext's wishlist twin.
// Lives in a Context (not a plain hook) so every component sees the same
// state — e.g. the header's cart badge updates the instant any page's
// "Add to Cart" button is clicked, without a page reload.
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <CartContext.Provider value={{ items, loading, addToCart, updateQuantity, removeFromCart, clearCart }}>
      {children}
